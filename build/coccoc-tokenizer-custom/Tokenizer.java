...
public class Tokenizer {
	...
	static {
		CodeSource codeSource = Tokenizer.class.getProtectionDomain().getCodeSource();
		try {
			String codeSourceLocation = codeSource.getLocation().toURI().getPath();
			File jarFile = new File(codeSourceLocation);
			String jarDir = jarFile.getParentFile().getPath();
			System.load(jarDir + "/coccoc-tokenizer/libcoccoc_tokenizer_jni.so");
	...
	private static final class Loader {
		...
		private static Tokenizer get() {
			...
			try {
				codeSourceLocation = codeSource.getLocation().toURI().getPath();
        	} catch (URISyntaxException e) {
        	}
			File jarFile = new File(codeSourceLocation);
			String jarDir = jarFile.getParentFile().getPath();
			Tokenizer instance = new Tokenizer(jarDir + "/coccoc-tokenizer/dicts");
			return instance;
		}
	}
